// Stage 2 auto-grab UI tests — the per-mode grab-trigger flows: Movies direct
// grab, the manual fallback pick list, the Series season/episode picker gating,
// and Adult's runtime-sourced grab. Also the explicit no-bulk assertion: one
// click fires exactly one auto-grab for exactly one title.
...
describe("Discover auto-grab — Movies (direct one-click)", () => {
  ...
  it("shows the ranked manual pick list on fallback, and grabs one chosen release", async () => {
    ...
    // The manual grab targets exactly one release with the chosen root folder.
    ...
    // Still no auto-grab beyond the single triggering call.
    expect(autograbCalls(calls)).toHaveLength(1);
  });
});

describe("Discover auto-grab — Series (picker gates the grab)", () => {
  it("requires a season/episode selection before any release is auto-grabbed", async () => {
    const calls = stubFetch((url) => {
      if (url.includes("/api/modes/movies/discover")) return jsonResponse([]);

...
func TestClearAll_RemovesEveryMatchedEntityAndSeenRelease(t *testing.T) {
	s := newTestReleaseStore(t)
	ctx := context.Background()

	if err := s.Insert(ctx, MatchedRelease{RowType: RowScene, EntityID: "s1", EntitySource: "tpdb", EntityTitle: "Scene One"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if err := s.Insert(ctx, MatchedRelease{RowType: RowStudio, EntityID: "Studio One", EntitySource: "tpdb", EntityTitle: "Studio One"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if err := s.MarkSeen(ctx, "seen-guid"); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	scenes, err := s.List(ctx, RowScene, "", 1, 20)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(scenes) != 0 {
		t.Errorf("expected no scenes after ClearAll, got %+v", scenes)
	}
	studios, err := s.List(ctx, RowStudio, "", 1, 20)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(studios) != 0 {
		t.Errorf("expected no studios after ClearAll, got %+v", studios)
	}
	seen, err := s.SeenGUIDs(ctx, []string{"seen-guid"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if seen["seen-guid"] {
		t.Errorf("expected seen-guid to be cleared")
	}
}
